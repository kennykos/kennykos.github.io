...
The energy maximization principle was originally introduced in a sequence of papers written by {% cite Slepian_Pollak_1961 %} and {% cite Landau_Pollak_1961 %} at Bell Laboratories as a direct corollary to a novel proof of Heisenberg's *uncertainty principle*. Though the authors of these papers studied PSWFs as the eigenvalues of the bandlimiting operator 
$$
\begin{equation}
B_c[f](t) = \int_{-1}^1 f(\omega)e^{ict\omega}\,\mathrm{d}t,
\end{equation}
$$
spheroidal wave functions have a rich lineage in mathematical physics, where they where introduced by {% cite Niven_Glaisher_1879%} as a tool to solve the Helmholtz equation in prolate spheroidal coordinates via separation of variables in the investigation of heat conduction of an ellipsoid of revolution. The first chapter of {% cite alma991020983909706011%} provides an excellent historical survey of the study of spheroidal wave functions up until the publishing of the papers from Slepian, Pollak and Landau, and {% cite Osipov_Rokhlin_Xiao_2013 %} serves as a terrific reference for the modern study of PSWFs.
...
# Notation
Consider the space $$L^2 = L^2(\mathbb R; \mathbb C)$$ of complex valued square integrable functions defined over the real number line. We adopt the notation that 
